...
def sample_ingredient(user, name="Pork"):
...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_partial_updating_recipes(self):
        """Test we can update a partial update"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
    ...
    def test_full_updating_recipes(self):
        """Test we can update with a put update"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        payload = {
            'title': 'Spag Bol',
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self):
        """Test results filtered by tags"""
        recipe1 = sample_recipe(user=self.user, title="Thai veg curry")
        recipe2 = sample_recipe(user=self.user, title="Aub with tahini")
        tag1 = sample_tag(user=self.user, name="Vegan")
        tag2 = sample_tag(user=self.user, name="Vegetarian")
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = sample_recipe(user=self.user, title="Meat Pie")
    ...
    def test_filter_recipes_by_ingredients(self):
        """Test results filtered by ingredientss"""
        recipe1 = sample_recipe(user=self.user, title="Bens on toast")
        recipe2 = sample_recipe(user=self.user, title="Chicken Stuff")
